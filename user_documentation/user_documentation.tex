...
\begin{itemize}
  ...
  \end{lstlisting}
  \item Generate a natural strategy for the given model:
  \begin{lstlisting}
    --NATURAL_STRATEGY
  \end{lstlisting}
  \item Set a strategy to the one given in a file:
  \begin{lstlisting}
    --STRATEGY_FROM_FILE [file path]
  \end{lstlisting}
\end{itemize}
...
\subsubsection{Probabilistic actions}
If the action is supposed to yield different results in a probabilistic manner, we need to write them similarly to the normal actions but with the addition of the probability as a number with a decimal dot and a colon, which should be between the arrow and the state that we want to reach. Then in each new line we can only use the probability, colon and the state that we want to reach. The precise syntax is as in the following example:
\begin{lstlisting}[title={Action A that leads from a state q0 to q1 with a probability of 0.8, to q2 with a probability of 0.15 and to q3 with a probability signified by a keyword $\&REST$ which signifies the leftover probability, which in this case would be equal to 0.05.}]
  A: q0 -> 0.80 : q1
           0.15 : q2
           &REST : q3
\end{lstlisting}
...
\begin{itemize}
  ...
  \item $\&H{\_}agent[<=k/>=k]((formula1), (formula2), ...)$ -- Hartley entropy (Agent name needed after the "{\_}". Then a $<=$ or $>=$ followed by an integer are required, wrapped in square brackets. Lastly, you write the formulas that you want to check in round brackets. Allows for listing more than one formula, separated with commas, each formula encapsulated within round brackets.)
\end{itemize}

\subsubsection{Probabilistic formulas}
Probabilistic verification is enabled automatically when the formula contains a probability constraint. Use the \verb|&P[op value]| fragment between the temporal operator and the formula body, where \verb|op| is one of:
\begin{lstlisting}
>  >=  <  <=
\end{lstlisting}

\begin{lstlisting}[title={Examples of probabilistic formulas.}]
FORMULA: <<Agent1>><>&P[>=0.75](goal==1)
FORMULA: <<Agent1>>[]&P[<0.20](error==1)
\end{lstlisting}

Probabilistic formulas are evaluated over probabilistic transitions (see Probabilistic actions). For a satisfied query, the tool prints both the boolean verdict and the computed value (\verb|p=...|).
...
\begin{itemize}
  ...
  \item $\&H{\_}agent[<=k/>=k]((formula1), (formula2), ...)$ -- Hartley entropy
  \item $\&P[==/!=/>/>=/</<=value]$ -- Probability threshold for a formula
\end{itemize}
